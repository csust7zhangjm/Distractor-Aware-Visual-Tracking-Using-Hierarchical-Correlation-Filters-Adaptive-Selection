function [results,time] = trackerMain2_wy(p, im, bg_area, fg_area, area_resize_factor)
...
%              response=expert(ID(1)).response+0.5*expert(ID(2)).response;
              response=expert(3).response+0.5*expert(2).response++0.25*expert(1).response;
...
%          Score(frame)=calculatePSR(response);
%       else
...
%                  mySubplot(2,1,5,1,hann_window_cosine,'19','parula');
%                  mySubplot(2,1,5,1,expert(1).response,'19','parula');
%                  mySubplot(2,1,5,3,expert(2).response,'28','parula');
%                  mySubplot(2,1,5,4,expert(3).response,'37','parula');
%                 mySubplot(2,1,5,5,response,'merged','parula');
                drawnow
      end
  end
