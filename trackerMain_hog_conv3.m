function [results,time] = trackerMain_hog_conv3(p, im, bg_area, fg_area, area_resize_factor)
...
for frame = 1:num_frames
   if frame>1
      ...
      % Middle-level feature (conv3-3) based DCF
      ...
      expert(2).response = responseDeepLow;
      expert(3).response = responseDeepMiddle;
      expert(4).response = responseDeepHigh;
      
      Score1 = calculatePSR(expert(1).response); 
      Score2 = calculatePSR(expert(2).response);
      Score3 = calculatePSR(expert(3).response);
      Score4 = calculatePSR(expert(4).response);
      ...
      response=expert(ID(1)).response+0.5*expert(ID(2)).response;
...
%          Final_rect_position = expert(7).rect_position(frame,:);
      ...
      %% ADAPTIVE UPDATE
       Score1_psr = calculatePSR(response_cfilter);           
       Score2_psr = calculatePSR(response_deep{1});         
       Score3_psr = calculatePSR(response_deep{2});
%       Score4 = calculatePSR(response_deep{3});
%       PSRScore(frame) = (Score1 + Score2 + Score3+Score4)/4;
       PSRScore(frame) = (Score1_psr + Score2_psr + Score3_psr)/3;
